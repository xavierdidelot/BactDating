...
test_that("Short MCMC runs give same results for C++ and R likelihoods.", {
  ...
  set.seed(0)
  res=bactdate(phy,leaves,nbIts=10,model='gamma')
  set.seed(0)
  res2=bactdate(phy,leaves,nbIts=10,model='gammaR')
  expect_equal(res$record[10,'likelihood'],res2$record[10,'likelihood'])
  set.seed(0)
...
test_that("Likelihood in C++ and R give identical results on recombinant tree.", {
  ...
  res=bactdate(phy,leaves,nbIts=2,model='gamma',useRec=T)
  res2=bactdate(phy,leaves,nbIts=2,model='gammaR',useRec=T)
  expect_equal(res$record[1,'likelihood'],res2$record[1,'likelihood'])
...
test_that("MCMC likelihood remains correct after many partial updates.", {
  ...
  phy=simobsphy(tree)
  set.seed(0)
  res=bactdate(phy,leaves,nbIts=10,thin=10,model='gamma',initRate=5.2,updateRate=F,updateRatevar=F,updateRoot=F)
